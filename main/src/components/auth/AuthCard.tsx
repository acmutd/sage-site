...
const AuthCard = (props: AuthCardProps) => {
  return (
    <div>
    {props.mobileView ? 
    <div className="flex min-h-screen h-max dark justify-center [@media(max-height:720px)]:items-start items-center [@media(max-height:720px)]:py-16">
      <div className="[@media(max-height:720px)]:block hidden fixed top-0 left-0 w-screen h-16 bg-bgdark"></div>
      <div className="flex-row w-[75vw] [@media(max-height:720px)]:w-[90vw] h-max min-h-[70vh] rounded-md overflow-hidden">
        <div className="flex-[4] w-auto bg-[#E4E4E4] flex justify-center items-center px-16 py-8">
          <img
        ...
        <div className="flex-[6] bg-bglight rounded-bl-md rounded-br-md">
          <div className="flex flex-col justify-center h-full w-full mx-auto px-[2rem] py-8">
            <h3 className="text-[18px] font-bold text-textdark mb-4">
              {props.title}
            </h3>
            {props.form}
          </div>
