...
import { X, TriangleAlert, CheckCircle, Calendar, Plus, Trash2, SlidersHorizontal, ChevronDown, ChevronUp } from 'lucide-react';
...
const SchedulePlanningModal: React.FC<SchedulePlanningModalProps> = ({ title, courses, onClose }) => {
    ...
    // conversion to minutes -- basically..give student freedom for padding
    const classPaddingMinutes = useMemo(() => {
        if (customPadding !== '') {
            const val = parseFloat(customPadding);
            if (!isNaN(val)) return paddingUnit === 'hr' ? Math.round(val * 60) : Math.round(val);
        }
        return classPadding;
    ...
    const professorDropdownWidth = useMemo(() => {
        ...
        return maxLen * 7 + 48; // ~7px per char + padding + chevron
    }, [plannableCourses]);
    ...
    return ReactDOM.createPortal(
        <>
            <div className="fixed inset-0 bg-black bg-opacity-40 z-[9998]" onClick={() => { if (Object.keys(selectedSections).length === 0) onClose(); }} />
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 pointer-events-none">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col pointer-events-auto"
                    ...
                    {/* Filter bar */}
                    {showFilters && (
                        <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex flex-wrap gap-4 items-end">
                                ...
                                <div key={code} className="px-5 py-4">
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {courseProfessors.length > 1 && (
                                                <div className="relative">
                                                    <select
                                                        ...
                                                        className="text-xs border border-gray-200 rounded-md px-2.5 py-1 pr-7 bg-white appearance-none focus:outline-none focus:ring-1 focus:ring-green-400 max-w-[150px]">
                                            ...
                                            {selectedKey && (
                                                <span className="text-[11px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium truncate">
                    ...
                    <div className="px-5 py-3 border-t border-gray-200 flex items-center justify-between bg-gray-50 rounded-b-xl">
                        ...
                        <button onClick={onClose}
                            className="px-4 py-1.5 text-sm bg-accent rounded-md hover:bg-gray-700 transition-colors">
                            Done
                        </button>
                    </div>
                </div>
